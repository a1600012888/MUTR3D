...
def giou_3d(box3d1:torch.Tensor, box3d2:torch.Tensor, enclosing_type:str="smallest", aligned=True):
    """calculated 3d GIoU loss. assume the 3d bounding boxes are only rotated around z axis
    ...
    """
    box3d1 = transform_box(box3d1)
    box3d2 = transform_box(box3d2)

    M = box3d1.size(0)
    N = box3d2.size(0)

    if aligned:
        box3d1 = box3d1.view(1, M, -1)
        box3d2 = box3d2.view(1, N, -1) 
        giou_loss, iou3d = cal_giou_3d(box3d1, box3d2, enclosing_type)
        giou_loss = giou_loss.view(N)
        iou3d = iou3d.view(N)
        return giou_loss, iou3d
    else:
        box3d1 = box3d1.view(1, M, 1, -1).repeat(1, 1, N, 1).view(1, M*N, -1)
        box3d2 = box3d2.view(1, 1, N, -1).repeat(1, M, 1, 1).view(1, M*N, -1)
        giou_loss, iou3d = cal_giou_3d(box3d1, box3d2, enclosing_type)
        giou_loss = giou_loss.view(M, N)
        iou3d = iou3d.view(M, N)
        return giou_loss, iou3d


def transform_box(box3d):
    x = box3d[:, 0:1]
    y = box3d[:, 1:2]
    z = box3d[:, 2:3]
    w = box3d[:, 3:4]
    l = box3d[:, 4:5]
    h = box3d[:, 5:6]
    alpha = -box3d[:, 6:7] - np.pi / 2    
    return torch.cat((y, x, z, l, w, h, alpha), -1)
