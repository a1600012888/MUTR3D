...
class DETR3D(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        if teacher is not None:
            ckpt = torch.load(teacher, map_location='cpu')
            print('loading ckpt (teacher) from:', teacher)
            if pts_voxel_encoder:
                pts_voxel_encoder_state_dict = OrderedDict()
                for key in ckpt['state_dict']:
                    if 'pts_voxel_encoder' in key:
                         pts_voxel_encoder_state_dict[key.replace('pts_voxel_encoder.', '')] = ckpt['state_dict'][key]
                self.pts_voxel_encoder_t = builder.build_voxel_encoder(pts_voxel_encoder)
                self.pts_voxel_encoder_t.load_state_dict(pts_voxel_encoder_state_dict)
            if pts_middle_encoder:
                pts_middle_encoder_state_dict = OrderedDict()
                for key in ckpt['state_dict']:
                    if 'pts_middle_encoder' in key:
                         pts_middle_encoder_state_dict[key.replace('pts_middle_encoder.', '')] = ckpt['state_dict'][key]
                self.pts_middle_encoder_t = builder.build_middle_encoder(pts_middle_encoder)
                self.pts_middle_encoder_t.load_state_dict(pts_middle_encoder_state_dict)
            if pts_backbone:
                pts_backbone_state_dict = OrderedDict()
                for key in ckpt['state_dict']:
                    if 'pts_backbone' in key:
                         pts_backbone_state_dict[key.replace('pts_backbone.', '')] = ckpt['state_dict'][key]
                self.pts_backbone_t =  builder.build_backbone(pts_backbone)
                self.pts_backbone_t.load_state_dict(pts_backbone_state_dict)
        else:
            self.pts_voxel_encoder_t = None
            self.pts_middle_encoder_t = None
            self.pts_backbone_t = None
    ...
    def forward_pts_train(self,
        ...
        """
        outs = self.pts_bbox_head(pts_feats[0])
