...
class CenterPointCam(MVXTwoStageDetector):
    ...
    def extract_img_feat(self, img, img_metas, img_depth, img_mask):
        ...
        if self.with_img_neck:
            img_feats_reshaped, depth, mask = self.img_neck(img_feats_reshaped, img_metas, img_depth, img_mask)
        return img_feats_reshaped, depth, mask

    def extract_feat(self, points, img, img_metas, img_depth, img_mask):
        """Extract features from images and points."""
        if self.finetune:
            with torch.no_grad():    
                pts, pred_depth, pred_mask = self.extract_img_feat(img, img_metas, img_depth, img_mask)
        else:
            pts, pred_depth, pred_mask = self.extract_img_feat(img, img_metas, img_depth, img_mask)
        pts_feats = self.extract_pts_feat(pts, img_metas, None)
        return (None, pts_feats, pred_depth, pred_mask)
    ...
    def forward_train(self,
        ...
        """
        img_feats, pts_feats, pred_depth, pred_mask = self.extract_feat(
            points, img=img, img_metas=img_metas, img_depth=img_depth, img_mask=img_mask)
        losses = dict()
        losses_pts = self.forward_pts_train(pts_feats, gt_bboxes_3d,
                                            gt_labels_3d, img_metas,
                                            gt_bboxes_ignore)
        losses.update(losses_pts)
        losses_depth_and_mask = self.img_neck.loss(img_depth, img_mask, pred_depth, pred_mask)
        losses.update(losses_depth_and_mask)
        return losses
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False, img_depth=None, img_mask=None):
    ...
    def aug_test(self, points, img_metas, imgs=None, rescale=False, img_depth=None, img_mask=None):
        """Test function with augmentaiton."""
        img_feats, pts_feats, _, _ = self.extract_feats(points, img_metas, imgs, img_depth, img_mask)
        bbox_list = dict()
        pts_bbox = self.aug_test_pts(img_feats[0], img_metas, rescale)
