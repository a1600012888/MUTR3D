...
class Deformable3DDetrTransformer(Transformer):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(Deformable3DDetrTransformer, self).__init__(**kwargs)
    ...
    def init_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, MultiScaleDeformableAttention) or isinstance(m, MultiScaleDeformableAttentionSigmoid):
                m.init_weight()
...
class DGCNNAttn(BaseModule):
    ...
    def __init__(self,
        ...
        self.conv2 = nn.Sequential(nn.Conv2d(self.embed_dims*2, self.embed_dims, kernel_size=1, bias=False),
                                   nn.BatchNorm2d(self.embed_dims),
                                   nn.ReLU(inplace=True))
        self.conv3 = nn.Sequential(nn.Conv2d(self.embed_dims*2, self.embed_dims, kernel_size=1, bias=False),
                                   nn.BatchNorm2d(self.embed_dims),
                                   nn.ReLU(inplace=True))
        self.K = kwargs['K']
...
@ATTENTION.register_module()
class MLPAttn(BaseModule):
    """A warpper for MLP-type self-attention.
    Args:
        embed_dims (int): The embedding dimension.
        num_heads (int): Parallel attention heads. Same as
            `nn.MultiheadAttention`.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MLPAttn, self).__init__(init_cfg)
        self.embed_dims = embed_dims
        self.num_heads = num_heads
        self.dropout = dropout
        self.conv1 = nn.Sequential(nn.Conv1d(self.embed_dims, self.embed_dims, kernel_size=1, bias=False),
                                   nn.BatchNorm1d(self.embed_dims),
                                   nn.ReLU(inplace=True))
        self.conv2 = nn.Sequential(nn.Conv1d(self.embed_dims, self.embed_dims, kernel_size=1, bias=False),
                                   nn.BatchNorm1d(self.embed_dims),
                                   nn.ReLU(inplace=True))
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        query = query.permute(1, 2, 0) # [bs, embed_dims, num_queries]
        out = self.conv2(self.conv1(query))
        out = out.permute(2, 0, 1)
        return residual + self.dropout(out)


@ATTENTION.register_module()
class PseudoAttention(BaseModule):
    """An pseudo attention module. 
    ...
    def forward(self,
                ...
                **kwargs):
        """Forward Function of PseudoAttention.
        Args:
...
@ATTENTION.register_module()
class MultiScaleDeformableAttentionSigmoid(MultiScaleDeformableAttention):
...
@TRANSFORMER.register_module()
class Detr3DCamTransformer(BaseModule):
    """Implements the DeformableDETR transformer.
    Args:
        as_two_stage (bool): Generate query from encoder features.
            Default: False.
    ...
    def __init__(self,
                 ...
                 decoder=None,
                 **kwargs):
        super(Detr3DCamTransformer, self).__init__(**kwargs)
...
@TRANSFORMER_LAYER_SEQUENCE.register_module()
class Detr3DCamTransformerDecoder(TransformerLayerSequence):
    """Implements the decoder in DETR transformer.
    Args:
        return_intermediate (bool): Whether to return intermediate outputs.
        coder_norm_cfg (dict): Config of last normalization layer. Default：
    ...
    def __init__(self, *args, return_intermediate=False, **kwargs):
        ...
        super(Detr3DCamTransformerDecoder, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate
...
@ATTENTION.register_module()
class Detr3DCamCrossAtten(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 norm_cfg=None,
                 init_cfg=None):
        super(Detr3DCamCrossAtten, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
